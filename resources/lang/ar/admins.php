<?php

return [
    'create_title' => 'إنشاء مدير',
    'back' => 'رجوع',
    'name' => 'الاسم',
    'email' => 'البريد الإلكتروني',
    'password' => 'كلمة المرور',
    'save' => 'حفظ',
    'edit_title' => 'تعديل المسؤول',
    'leave_blank' => 'اتركه فارغًا إذا لم ترغب في التغيير',
    'password_hint' => 'أدخل كلمة مرور جديدة لتغييرها',
    'update' => 'تحديث',
    'login_title' => 'تسجيل دخول الإدارة',
    'remember_me' => 'تذكرني',
    'login_btn' => 'تسجيل الدخول',
    // Register
    'create_account' => 'إنشاء حساب مدير',
    'confirm_password' => 'تأكيد كلمة المرور',
    'register_btn' => 'إنشاء الحساب',
    'users' => 'إجمالي المستخدمين',
    'clients' => 'إجمالي العملاء',
    'deals' => 'إجمالي الصفقات',
    'login_title' => 'مرحباً بك مجدداً',
    'login_subtitle' => 'قم بتسجيل الدخول إلى حساب NexusCRM لإدارة مبيعاتك.',
    'email' => 'البريد الإلكتروني للعمل',
    'password' => 'كلمة المرور',
    'remember_me' => 'البقاء قيد تسجيل الدخول لمدة 30 يوماً',
    'login_btn' => 'تسجيل الدخول للوحة التحكم',
    'forgot_password' => 'نسيت كلمة المرور؟',



    // Dashboard
    'users' => 'المستخدمون',
    'clients' => 'العملاء',
    'leads' => 'العملاء المحتملون',
    'deals' => 'الصفقات',

    // لوحة القيادة: الفريق / الأهداف
    'team_performance_monitor' => 'مراقبة أداء الفريق',
    'member_name'              => 'اسم العضو',
    'target_total'             => 'إجمالي الهدف',
    'reached'                  => 'المبلغ المحقق',
    'progress'                 => 'التقدم',
    'status'                   => 'الحالة',
    'finished'                 => 'مكتمل',
    'no_target'                => 'لا يوجد هدف',
    'in_progress'              => 'قيد التنفيذ',
    'view_team'                => 'عرض الفريق',

    'recent_activities' => 'أحدث الأنشطة',
    'recent_activities_desc' => 'يمكنك عرض أحدث أنشطة العملاء أو الصفقات هنا.',
    'title'               => 'المسؤولون',
    'create_button'       => 'إنشاء مسؤول',
    'edit' => 'تعديل',
    'delete' => 'حذف',
    'dashboard' => 'لوحة التحكم',
    'no_team_data' => 'لا توجد بيانات متاحة لأعضاء الفريق.',
    'title' => 'المسؤولين',
    'edit_title' => 'تعديل حساب المسؤول',
    'name' => 'الاسم',
    'email' => 'البريد الإلكتروني',
    'password' => 'كلمة المرور',
    'confirm_password' => 'تأكيد كلمة المرور',
    'leave_blank' => 'اتركه فارغاً للحفاظ على الحالي',
    'password_hint' => 'اترك هذا الحقل فارغاً إذا كنت لا ترغب في تغيير كلمة المرور.',
    'role' => 'الدور',
    'permissions' => 'الصلاحيات',
    'select_all' => 'تحديد الكل',
    'deselect_all' => 'إلغاء تحديد الكل',
    'no_permissions_available' => 'لا توجد صلاحيات متاحة. يرجى تشغيل ملف تهيئة الصلاحيات (Seeder).',
    'permissions_help' => 'اختر الصلاحيات لهذا الدور. يتم تعيين الصلاحيات للأدوار وليس للمسؤولين بشكل فردي.',
    'update' => 'تحديث حساب المسؤول',

    // مصطلحات إضافية قد تحتاجها
    'create_title' => 'إنشاء مسؤول جديد',
    'save' => 'حفظ التغييرات',
    'cancel' => 'إلغاء',
    'dashboard' => 'لوحة التحكم',

];
